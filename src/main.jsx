...
function Library({ selectedId, onSelect }) {
...
function Diagram({ frets }) {
  return <div className="diagram" aria-label="Chord diagram"><div className="diagram-head"><span>CHORD DIAGRAM</span><span>6 5 4 3 2 1</span></div><div className="diagram-nut" />{Array.from({ length: 6 }, (_, index) => <div className="diagram-string" key={index} style={{ left: `${12 + index * 14.8}%` }}><span className={`diagram-note ${frets[index] === "muted" ? "muted" : ""}`}>{frets[index] === "muted" ? "×" : frets[index] === null ? "" : frets[index] === 0 ? "○" : "●"}</span></div>)}{Array.from({ length: 5 }, (_, index) => <div className="diagram-fret" key={index} style={{ top: `${28 + index * 14.5}%` }} />)}</div>;
}
...
function ShapeEditor({ selected, frets, setFrets, onAddToProgression, embedded = false }) {
  ...
  return <div className={`${embedded ? "shape-editor" : "workspace shape-editor"}`}>
    ...
    <section className="below-grid"><Diagram frets={frets} /><div className="voicing-details"><div className="details-heading"><div><p className="kicker">VOICING DETAILS</p><h2>{isEdited ? detected.label : `${selected.root}${selected.suffix}`} anatomy</h2></div><span className="source-tag">{selected.source}</span></div><div className="interval-grid">{selected.intervals.map((interval, index) => <div key={interval} className="interval"><span>{index === 0 ? "ROOT" : `INTERVAL ${index + 1}`}</span><strong>{interval === 0 ? "1" : interval === 3 ? "♭3" : interval === 4 ? "3" : interval === 7 ? "5" : interval === 10 ? "♭7" : interval === 11 ? "7" : interval}</strong></div>)}</div><p className="details-copy">This is a common {selected.shapeName.toLowerCase()} used in standard guitar voicing. Every note is shown above at its actual string and fret position.</p></div></section>
  </div>;
}

function LibraryPage({ selected, frets, setFrets, setSelected, addToProgression }) {
  return <div className="app-grid"><Library selectedId={selected.id} onSelect={(entry) => { setSelected(entry); setFrets(entry.frets); }} /><ShapeEditor selected={selected} frets={frets} setFrets={setFrets} onAddToProgression={() => addToProgression({ ...selected, frets: [...frets] })} /></div>;
}
